...
import pickle
import copy
from . import accuracy, activation, layer, loss, optimizer
...
class Model:
  ...
  def finalize(self):
    self.input_layer = layer.Layer_Input()
    ...
    if isinstance(self.layers[-1], activation.Activation_Softmax) and \
      isinstance(self.loss, loss.Loss_CategoricalCrossentropy):
      self.softmax_classifier_output = \
        loss.Activation_Softmax_Loss_CategoricalCrossentropy()
  def train(self, X, y, *, epochs=1, batch_size=None, print_every=1000, validation_data=None):
